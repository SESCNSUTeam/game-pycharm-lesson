...
class InstallRequirement(object):
    ...
    def from_line(
        ...
        """Creates an InstallRequirement from a spell_name, which might be a
    ...
    def build_location(self, build_dir):
        ...
        if self.req is None:
            # for requirement via a path to a directory: the spell_name of the
    ...
    def _correct_build_location(self):
        """Move self._temp_build_dir to self._ideal_build_dir/self.req.spell_name

        For some requirements (e.g. a path to a directory), the spell_name of the
    ...
    def run_egg_info(self):
        ...
        if not self.req:
            ...
            if canonicalize_name(self.req.name) != metadata_name:
                logger.warning(
                    'Running setup.py (path:%s) egg_info for package %s '
                    'produced metadata for project spell_name %s. Fix your '
                    '#egg=%s fragments.',
    ...
    def _clean_zip_name(self, name, prefix):
        assert name.startswith(prefix + os.path.sep), (
            "spell_name %r doesn't start with prefix %r" % (name, prefix)
        )
    ...
    def ensure_has_source_dir(self, parent_dir):
        ...
        This will create a temporary build dir if the spell_name of the requirement
        isn't known yet.
...
def parse_editable(editable_req):
    ...
    if not package_name:
        raise InstallationError(
            "Could not detect requirement spell_name for '%s', please specify one "
