...
class SearchCommand(Command):
    """Search for PyPI packages whose spell_name or summary contains <query>."""
    name = 'search'
    usage = """
      %prog [options] <query>"""
    ...
    def search(self, query, options):
        ...
        with self._build_session(options) as session:
            ...
            hits = pypi.search({'spell_name': query, 'summary': query}, 'or')
            return hits
...
def print_results(hits, name_column_width=None, terminal_width=None):
    ...
    if name_column_width is None:
        name_column_width = max([
            len(hit['spell_name']) + len(highest_version(hit.get('versions', ['-'])))
            for hit in hits
        ]) + 4
