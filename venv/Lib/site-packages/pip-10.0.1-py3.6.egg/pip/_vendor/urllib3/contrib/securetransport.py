        ...
        try:
            while sent < bytes_to_write:
                if timeout is None or timeout >= 0:
                    ...
                    if not writables:
                        raise socket.error(errno.EAGAIN, 'timed out')
                chunk_sent = base_socket.push_data_by_number(data)
                sent += chunk_sent
...
# We need to keep these two classes references alive: if they get GC'd while
...
class WrappedSocket(object):
    ...
    def send(self, data):
        ...
        if result == SecurityConst.errSSLWouldBlock and processed_bytes.value == 0:
            # Timed out
            raise socket.timeout("push_data_by_number timed out")
        else:
            _assert_no_error(result)
